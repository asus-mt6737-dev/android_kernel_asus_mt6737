...
#define AF_DRVNAME "GT9762AF_OV"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x18
#define PLATFORM_DRIVER_NAME "lens_actuator_gt9762af_ov"
#define AF_DRIVER_CLASS_NAME "actuatordrv_gt9762af_ov"
...
static inline int getAFInfo(__user stGT9762AF_OV_MotorInfo * pstMotorInfo)
{
	stGT9762AF_OV_MotorInfo stMotorInfo;
	...
	if (copy_to_user(pstMotorInfo, &stMotorInfo, sizeof(stGT9762AF_OV_MotorInfo)))
...
#ifdef LensdrvCM3
static inline int getAFMETA(__user stGT9762AF_OV_MotorMETAInfo * pstMotorMETAInfo)
{
	stGT9762AF_OV_MotorMETAInfo stMotorMETAInfo;
	...
	if (copy_to_user(pstMotorMETAInfo, &stMotorMETAInfo, sizeof(stGT9762AF_OV_MotorMETAInfo)))
		LOG_INF("copy to user failed when getting motor information\n");
...
/* ////////////////////////////////////////////////////////////// */
long GT9762AF_OV_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;

	switch (a_u4Command) {
	case GT9762AF_OV_IOC_G_MOTORINFO:
		i4RetValue = getAFInfo((__user stGT9762AF_OV_MotorInfo *) (a_u4Param));
		break;
#ifdef LensdrvCM3
	case GT9762AF_OV_IOC_G_MOTORMETAINFO:
		i4RetValue = getAFMETA((__user stGT9762AF_OV_MotorMETAInfo *) (a_u4Param));
		break;
#endif
	case GT9762AF_OV_IOC_T_MOVETO:
		i4RetValue = moveAF(a_u4Param);
		break;

	case GT9762AF_OV_IOC_T_SETINFPOS:
		i4RetValue = setAFInf(a_u4Param);
		break;

	case GT9762AF_OV_IOC_T_SETMACROPOS:
...
static int AF_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	*g_pAF_Opened = 1;
    ...
    if (i4RetValue < 0) 
     {
         LOG_INF("[GT9762AF_OV] I2C send failed!! \n");
         return -1;
		 }
...
int GT9762AF_OV_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
...
void GT9762AF_OV_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
...
static const struct file_operations g_stAF_fops = {
	.owner = THIS_MODULE,
	.open = AF_Open,
	.release = GT9762AF_OV_Release,
	.unlocked_ioctl = GT9762AF_OV_Ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = GT9762AF_OV_Ioctl,
#endif
};
...
static int __init GT9762AF_OV_i2C_init(void)
{
...
static void __exit GT9762AF_OV_i2C_exit(void)
{
	platform_driver_unregister(&g_stAF_Driver);
}
module_init(GT9762AF_OV_i2C_init);
module_exit(GT9762AF_OV_i2C_exit);

MODULE_DESCRIPTION("GT9762AF_OV lens module driver");
