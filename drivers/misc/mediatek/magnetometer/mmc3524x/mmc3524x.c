...
static int I2C_TxData(char *txData, int length)
{
	...
	for(loop_i = 0; loop_i < MMC3524X_RETRY_COUNT; loop_i++)
	{
		...
		printk("I2C_TxData delay!this_client->addr = %d\n",this_client->addr );
		mdelay(10);
	}
...
static int mmc3524x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  printk(KERN_ERR "mmc3524x_i2c_set_clientdata\n");
	...
	MMCDBG("%s: OK\n", __func__);
	printk(KERN_ERR "mmc3524X IIC probe successful !");
