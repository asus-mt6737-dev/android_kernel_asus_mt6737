...
static void otg_chg_thread_func(void)
{
	if(otg_chg_flag==1)
	{
        //modify by qiangang@wind-mobi.com 20170314 begin
        mutex_lock(&otg_chg_mutex);
		ncp1854_set_chg_en(0);    //modify by qiangang@wind-mobi.com 2017112
		ncp1854_set_otg_en(1);
		msleep(50); 
	//	printk("qiangang OTG mode.\n");
	...
	{
		mutex_lock(&otg_chg_mutex);
		ncp1854_set_chg_en(0);    //modify by qiangang@wind-mobi.com 2017112s
        ncp1854_set_otg_en(0);                
		 msleep(50);               
		//ncp1854_set_chg_en(0);    //modify by qiangang@wind-mobi.com 2017112
		mutex_unlock(&otg_chg_mutex);		
		kthread_stop(otg_chg_thread);
        //modify by qiangang@wind-mobi.com 20170314	end
	}
...
static size_t _otg_chg_store(struct otg_chg_device *dev,const char * buf,size_t count)
{
	      int val=0;
...
static void otg_chg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{

	struct otg_chg_device *gdev=(struct otg_chg_device*)dev_get_drvdata(dev);
	
	 _otg_chg_store(gdev,buf,size);
