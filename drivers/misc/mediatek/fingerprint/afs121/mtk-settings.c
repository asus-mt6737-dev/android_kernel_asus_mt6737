...
*/
 	mas_spi_mi_on = pinctrl_lookup_state(mas_finger_pinctrl, "spi_mi_low");
 ...
 */	mas_spi_mo_on = pinctrl_lookup_state(mas_finger_pinctrl, "spi_mo_low");
...
*/
	mas_spi_ck_on = pinctrl_lookup_state(mas_finger_pinctrl, "spi_mclk_low");
...
*/
	mas_spi_cs_on = pinctrl_lookup_state(mas_finger_pinctrl, "spi_cs_high");
...
*/
	mas_finger_eint_on = pinctrl_lookup_state(mas_finger_pinctrl, "eint");
	if (IS_ERR(mas_finger_eint_on)) {
		ret = PTR_ERR(mas_finger_eint_on);
...
unsigned int mas_get_interrupt_gpio(int index){
	unsigned int finger_int_pin;
	int val;
	val = of_property_read_u32_index(node, "finger_int_pin", index, &finger_int_pin);
	if(val < 0){
		return val;
	}
