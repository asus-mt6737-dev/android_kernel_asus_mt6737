...
//zhaozhensen@wind-mobi.com 20160406 begin
#define CAM_CALDB  pr_debug
#define OV5670OTP
//zhaozhensen@wind-mobi.com 20160406 end
...
static int RG_Ratio_Typical = 0x111;//0x19f;
static int BG_Ratio_Typical = 0x121;//0x163;

static int read_otp(struct otp_struct *otp_ptr)
{
	int otp_flag,addr,i, temp;
	...
	//OTP into
		...
		pr_debug("otp_info_otp_flag  = %x, addr = %x\r\n", otp_flag,addr);//bg
		 ...
		 pr_debug("module_integrator_id  = %x\r\n", (*otp_ptr).module_integrator_id);//bg
		 //OTP WB Calibration
...
int apply_ov5670_otp(struct otp_struct *otp_ptr)
{
       int R_gain, G_gain, B_gain, Base_gain, rg,bg;
...
static int update_otp(void)
{
	struct otp_struct current_otp;

	read_otp(&current_otp);
	apply_ov5670_otp(&current_otp);
...
{
	pr_debug("zzs dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
// zhaozhensen@wind-mobi.com 20160506 begin
...
// zhaozhensen@wind-mobi.com 20160506 end
...
{
	pr_debug("zzs OV5670_Sensor_Init_2lane E\n");
...
{
	pr_debug("zzs OV5670PreviewSetting_2lane enter\n");
...
{ 
	pr_debug("zzs hs_video_setting enter!\n");
...
{
	pr_debug("zzs slim_video_setting enter!\n");
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				pr_debug("zzs i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			pr_debug("zzs Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	pr_debug("zzs preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
...
{
	// zhaozhensen@wind-mobi.com 20160128 begin
	kal_uint32 frame_length = 0x0000;
