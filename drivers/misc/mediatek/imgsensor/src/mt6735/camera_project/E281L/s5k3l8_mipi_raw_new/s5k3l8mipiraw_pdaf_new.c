...
BYTE S5K3L8_eeprom_data_new[DATA_SIZE]= {0};
...
static bool _read_eeprom(kal_uint16 addr, kal_uint32 size ){
  ...
  int i=0;
  for(; i<size; i++){
    S5K3L8_eeprom_data_new[i] = read_cmos_sensor_byte(addr+i);
    LOG_INF("add = 0x%x,\tvalue = 0x%x",i, S5K3L8_eeprom_data_new[i]);
  }
  return true;
}

bool S5K3L8_read_eeprom_new( kal_uint16 addr, BYTE* data, kal_uint32 size){
	addr = START_ADDR;
	size = DATA_SIZE;
	...
	memcpy(data, S5K3L8_eeprom_data_new, size);
	return true;
}
