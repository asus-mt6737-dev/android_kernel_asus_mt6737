...
#define s5k3m2_OTP_FUNCTION
#define RG_Ratio_Typical 0xF9
#define BG_Ratio_Typical 0x113
extern int read_S5k3m2_otp_mtk_fmt(void);
...
kal_uint16 s5k3m2_otp_read_8(kal_uint16 addr)
{
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    kal_uint16 get_byte=0;
    char pusendcmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
...
void s5k3m2_otp_write_8(kal_uint16 addr, kal_uint8 para)
{
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    char pusendcmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,(char)(para & 0xFF)};
    iWriteRegI2C(pusendcmd , 3, imgsensor.i2c_write_id);
...
//ZHAOZHENSEN@WIND-MOBI.COM 20170101 begin
#ifdef s5k3m2_OTP_FUNCTION
int info_flag1 = 0;
int info_flag2 = 0;
void s5k3m2_OTP_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    char pusendcmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,(char)(para >> 8),(char)(para & 0xFF)};
    iWriteRegI2C(pusendcmd , 4, imgsensor.i2c_write_id);
...
static struct s5k3m2_otp_struct s5k3m2_otp_info;

static void s5k3m2_OTPSetting(void)
{
    LOG_INF("s5k3m2OTPSetting begin:\n ");
	msleep(1);
	...
	msleep(10);
    LOG_INF("s5k3m2OTPSetting exit :\n ");
}

void s5k3m2_Sensor_OTP_info(void)
{
    ...
    s5k3m2_OTP_write_cmos_sensor(0x0100, 0x0100);
	msleep(10);
	write_cmos_sensor_8(0x0A02, 0x1F); //select page
    write_cmos_sensor_8(0x0A00, 0x01); // read enable
	info_flag1 = read_cmos_sensor_8(0x0A08);
    info_flag2 = read_cmos_sensor_8(0x0A1D);
    printk("info_flag = 0x%x\n", info_flag1);
	if(info_flag1 == 0x01){
            ModuleHouseID = read_cmos_sensor_8(0x0A09);
            Year = read_cmos_sensor_8(0x0A0D);
            month = read_cmos_sensor_8(0x0A0E);
            day = read_cmos_sensor_8(0x0A0F);
            //sensor_id=read_cmos_sensor_8(0x0507,0,0);
     } else if((info_flag1 == 0xff)&&(info_flag2 == 0x01)) {
            ModuleHouseID = read_cmos_sensor_8(0x0A1E);
            Year = read_cmos_sensor_8(0x0A22);
            month = read_cmos_sensor_8(0x0A23);
            day = read_cmos_sensor_8(0x0A24);
            //sensor_id=read_cmos_sensor_8(0x0518,0,0);
	}
	write_cmos_sensor_8(0x0A00, 0x00);
    pr_err("zzs s5k3m2_ModuleHouseID = %x \n", ModuleHouseID);
    pr_err("zzs s5k3m2_Year = %d \n", Year);
    pr_err("zzs s5k3m2_month = %d \n", month);
    pr_err("zzs s5k3m2_day = %d \n", day);	
    //LOG_INF("s5k3m2_sensor_id = %d \n", sensor_id);
	  
}
int rg_data = 0;
int bg_data = 0;
void s5k3m2_Sensor_read_awbdata(void)
{
    uint16_t wbcheck = 0;
	...
	write_cmos_sensor_8(0x0A02, 0x1F); //select page
    write_cmos_sensor_8(0x0A00, 0x01); // read enable
	if(read_cmos_sensor_8(0x0A08) == 0x01){
            wb_unit_rg_h = read_cmos_sensor_8(0x0A10);
            wb_unit_rg_l = read_cmos_sensor_8(0x0A11);
            wb_unit_bg_h = read_cmos_sensor_8(0x0A12);
            wb_unit_bg_l = read_cmos_sensor_8(0x0A13);
            wb_unit_gg_h = read_cmos_sensor_8(0x0A14);
            wb_unit_gg_l = read_cmos_sensor_8(0x0A15);
            wb_golden_rg_h = read_cmos_sensor_8(0x0A16);
            wb_golden_rg_l = read_cmos_sensor_8(0x0A17);
            wb_golden_bg_h = read_cmos_sensor_8(0x0A18);
            wb_golden_bg_l = read_cmos_sensor_8(0x0A19);
            wb_golden_gg_h = read_cmos_sensor_8(0x0A1A);
            wb_golden_gg_l = read_cmos_sensor_8(0x0A1B);
     } else if (read_cmos_sensor_8(0x0A1D) == 0x01) {

            wb_unit_rg_h = read_cmos_sensor_8(0x0A25);
            wb_unit_rg_l = read_cmos_sensor_8(0x0A26);
            wb_unit_bg_h = read_cmos_sensor_8(0x0A27);
            wb_unit_bg_l = read_cmos_sensor_8(0x0A28);
            wb_unit_gg_h = read_cmos_sensor_8(0x0A29);
            wb_unit_gg_l = read_cmos_sensor_8(0x0A2A);
            wb_golden_rg_h = read_cmos_sensor_8(0x0A2B);
            wb_golden_rg_l = read_cmos_sensor_8(0x0A2C);
            wb_golden_bg_h = read_cmos_sensor_8(0x0A2D);
            wb_golden_bg_l = read_cmos_sensor_8(0x0A2E);
            wb_golden_gg_h = read_cmos_sensor_8(0x0A2F);
            wb_golden_gg_l = read_cmos_sensor_8(0x0A30);
    }
    write_cmos_sensor_8(0x0A00, 0x00); // read disable
    pr_err("zzs wb_unit_rg_h = 0x%x, wb_unit_rg_l = 0x%x, wb_unit_bg_h = 0x%x, wb_unit_bg_l = 0x%x, wb_golden_rg_h = 0x%x, wb_golden_rg_l = 0x%x, wb_golden_bg_h = 0x%x, wb_golden_bg_l = 0x%x,  wbcheck = 0x%x\n", 
	    wb_unit_rg_h, wb_unit_rg_l, wb_unit_bg_h, wb_unit_bg_l, wb_golden_rg_h, wb_golden_rg_l, wb_golden_bg_h, wb_golden_bg_l, wbcheck);
    printk("zzs wb_unit_rg_h = 0x%x, wb_unit_rg_l = 0x%x, wb_unit_bg_h = 0x%x, wb_unit_bg_l = 0x%x, wb_golden_rg_h = 0x%x, wb_golden_rg_l = 0x%x, wb_golden_bg_h = 0x%x, wb_golden_bg_l = 0x%x,  wbcheck = 0x%x\n", 
    wb_unit_rg_h, wb_unit_rg_l, wb_unit_bg_h, wb_unit_bg_l, wb_golden_rg_h, wb_golden_rg_l, wb_golden_bg_h, wb_golden_bg_l, wbcheck);
	...
	rg_data = rg_value;
	bg_data = bg_value;
    printk("zzs Read from module: rg_golden_value = 0x%x, bg_golden_value = 0x%x\n", rg_golden_value, bg_golden_value);
    printk("zzs Read from module: rg_value = 0x%x, bg_value = 0x%x\n",rg_value, bg_value);
    s5k3m2_otp_info.r_gain = (0x100 * RG_Ratio_Typical) / rg_value;
    s5k3m2_otp_info.b_gain = (0x100 * BG_Ratio_Typical) / bg_value;
    s5k3m2_otp_info.g_gain = 0x100;
    if(s5k3m2_otp_info.r_gain < s5k3m2_otp_info.b_gain)
        {
            if(s5k3m2_otp_info.r_gain < 0x100)
            {
                s5k3m2_otp_info.b_gain = 0x100 * s5k3m2_otp_info.b_gain / s5k3m2_otp_info.r_gain;
                s5k3m2_otp_info.g_gain = 0x100 * s5k3m2_otp_info.g_gain / s5k3m2_otp_info.r_gain;
                s5k3m2_otp_info.r_gain = 0x100;
            }
    }
    else
     {
            if(s5k3m2_otp_info.b_gain < 0x100)
            {
                s5k3m2_otp_info.r_gain = 0x100 * s5k3m2_otp_info.r_gain / s5k3m2_otp_info.b_gain;
                s5k3m2_otp_info.g_gain = 0x100 * s5k3m2_otp_info.g_gain / s5k3m2_otp_info.b_gain;
                s5k3m2_otp_info.b_gain = 0x100;
            }
    }
    printk("zzs s5k3m2 After calcuate: g_gain = 0x%x, r_gain = 0x%x,b_gain = 0x%x\n", s5k3m2_otp_info.g_gain,s5k3m2_otp_info.r_gain,s5k3m2_otp_info.b_gain);

}

void s5k3m2_Sensor_update_wb_gain(USHORT rg, USHORT bg, USHORT gg)
{
    USHORT r_gain, b_gain ,g_gain;
    uint16_t a05 = 0;

    s5k3m2_OTP_write_cmos_sensor(0x6028, 0x4000); //write data
    s5k3m2_OTP_write_cmos_sensor(0x602A, 0x3056); //write data
    write_cmos_sensor_8(0x6F12, 0x01);            //write data
    r_gain = rg;
    b_gain = bg;
    g_gain = gg;
    printk("zzs r_gain = 0x%x, b_gain = 0x%x\n",r_gain, b_gain);
    pr_err("zzs r_gain = 0x%x, b_gain = 0x%x\n",r_gain, b_gain);
    s5k3m2_OTP_write_cmos_sensor(0x020E, g_gain); //write data
    s5k3m2_OTP_write_cmos_sensor(0x0212, b_gain); //write data
    s5k3m2_OTP_write_cmos_sensor(0x0210, r_gain); //write data
    s5k3m2_OTP_write_cmos_sensor(0x0214, g_gain); //write data
}
static void s5k3m2_lsc_apply(void)
{
    write_cmos_sensor_8(0x0b00, 0x01); //write data
    write_cmos_sensor_8(0x0100, 0x01); //write data
}
static void s5k3m2_OTP_cali(void)
{
	printk("zzs get in s5k3m2_OTP_cali BEGIN\n");
		//s5k3m2_lsc_apply();  //liuying sensor auto load
        s5k3m2_Sensor_read_awbdata();
		read_S5k3m2_otp_mtk_fmt();
        //s5k3m2_Sensor_OTP_info();
        s5k3m2_Sensor_update_wb_gain( s5k3m2_otp_info.r_gain, s5k3m2_otp_info.b_gain, s5k3m2_otp_info.g_gain);

   	printk("zzs get out s5k3m2_OTP_cali END\n");
}

#endif
//ZHAOZHENSEN@WIMD-MOBI,COM  20170101 end
/*************************************************************************
* FUNCTION
*	night_mode
*
...
static void sensor_init(void)
{
  ...
  write_cmos_sensor(0x6F12, 0x0040); //OB must=64 when PDAF
  //write_cmos_sensor_8(0x0B00, 0x01);//write_cmos_sensor(0x0b00, 0x0180); //dynamic bpc must off when using PDAF
  write_cmos_sensor_8(0x0B00, 0x00);//liuying
...
//  write_cmos_sensor(0x3AFA, 0xFBB8);   //eliminated
  ...
  //write_cmos_sensor(0x3A92, 0x0807);//liuying 
  write_cmos_sensor_8(0x3A92, 0x06);//liuying
  write_cmos_sensor(0x3AFA, 0xFBF8);
  write_cmos_sensor(0x602A, 0x195E);
  write_cmos_sensor(0x6F12, 0x999F);//liuying 0x99BF
  write_cmos_sensor(0x602A, 0x19B6);
  write_cmos_sensor(0x6F12, 0x999F);//liuying 0x99BF
  write_cmos_sensor(0x602A, 0x1A0E);
  write_cmos_sensor(0x6F12, 0x999F);//liuying 0x99BF
  ...
  //write_cmos_sensor(0x6F12, 0x0040); //OB must=64 when PDAF
  //write_cmos_sensor(0x6F12, 0x0040); //OB must=64 when PDAF
  write_cmos_sensor(0x6F12, 0x0000); //OB must=64 when PDAF
  write_cmos_sensor(0x6F12, 0x0000); //OB must=64 when PDAF
  //liuying@wind-mobi.com module lsc ob=0 end
  write_cmos_sensor_8(0x0B00, 0x01);// write_cmos_sensor(0x0b00, 0x0000); //dynamic bpc must off when using PDAF
  //write_cmos_sensor_8(0x0B00, 0x00);//liuying
  write_cmos_sensor(0x602A, 0x3058); //liuying
  write_cmos_sensor(0x6F12, 0x0900); //liuying
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	// zhaozhensen@wind-mobi.com 20161231 begin
	s5k3m2_OTP_cali();
	// zhaozhensen@wind-mobi.com 20161231 end
	spin_lock(&imgsensor_drv_lock);
