// zhaozhensen@wind-mobi.com 20160217 begin
...
#define LOG_1 pr_debug("IMX258,MIPI 1LANE\n")
#define LOG_2 pr_debug("preview 1600*1200@30fps,600Mbps/lane; video 1600*1200@30fps,600Mbps/lane; 1600*1200@30fps,600Mbps/lane\n")
/****************************   Modify end    *******************************************/
#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
// zhaozhensen@wind-mobi.com 20160307 add begin
#ifdef CONFIG_MTK_CAM_CAL	
extern int read_imx258_eeprom_otp_mtk_fmt(void);
extern void read_imx258_eeprom_otp_awb(void);
#endif
...
static imgsensor_info_struct imgsensor_info = {
	...
	//zhaozhensen@wind_mobi.com 30160314 begin
		.pclk = 518400000,//480000000,//518400000,
	//zhaozhensen@wind_mobi.com 30160314 end
		.linelength = 5352,
	//zhaozhensen@wind_mobi.com 30160314 begin
	...
	.custom1 = {/*low light mode capture*/
		.pclk = 259200000,				//record different mode's pclk
		.linelength = 5352,				//record different mode's linelength
		.framelength = 1614,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
	}, //zhenglihong@wind-mobi.com 20160707 
	.slim_video = {/*VT Call*/
	...
	.custom1_delay_frame = 2,//enter slim video delay frame num //zhenglihong@wind-mobi.com 20160707 
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
	{{ 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // Preview
	 ...
	 { 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // video
	 //zhaozhensen@wind_mobi.com 30160314 begin
	 ...
	 { 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}};// low light mode wupeiyuan@wind-mobi.com 20160329
...
static void load_imx258_SPC_Data(void)
{
	...
	{
		//zhaozhensen@wind_mobi.com 30160314 begin
		if(!read_imx258_eeprom_SPC(0x0D04,IMX258MIPI_SPC_Data,126))
		//zhaozhensen@wind_mobi.com 30160314 end
		{
			pr_debug("load imx258 failed\n");
			return;
		}
		SPC_data_done = true;
	}
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
			 	...
			 	//zhaozhensen@wind_mobi.com 30160314 begin
				 pr_debug("imx258 type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF)", imx258_type);
			 	//zhaozhensen@wind_mobi.com 30160314 end
...
// zhaozhensen@wind-mobi.com 20160316 begin
...
{
	pr_debug("capture_setting_pdaf E\n");
	write_cmos_sensor(0x0100,0x00);
	mdelay(10);
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	pr_debug("imgsensor.ihdr_en in video mode:%d\n",imgsensor.ihdr_en);
...
//zhenglihong@wind-mobi.com 20160707 begin
...
//zhenglihong@wind-mobi.com 20160707 end
/*************************************************************************
* FUNCTION
*	get_imgsensor_id
*
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	// zhaozhensen@wind-mobi.com 20160307 add begin
        #ifdef CONFIG_MTK_CAM_CAL
				read_imx258_eeprom_otp_mtk_fmt();
        #endif
	// zhaozhensen@wind-mobi.com 20160307 add end
				return ERROR_NONE;
			}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->Custom1DelayFrame = imgsensor_info.custom1_delay_frame;//zhenglihong@wind-mobi.com 20160707 
	...
	if(imx258_type == IMX258_HDR_TYPE)
	//zhaozhensen@wind_mobi.com 20160504 begin
		sensor_info->PDAF_Support = 0; /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode, 3:PDAF Binning mode*/
	//zhaozhensen@wind_mobi.com 20160504 end
	else if(imx258_type == IMX258_BINNING_TYPE)
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			custom1(image_window, sensor_config_data); //zhenglihong@wind-mobi.com 20160707 
			break;
		default:
...
// zhaozhensen@wind-mobi.com 20160217 end