// zhaozhensen@wind-mobi.com 20160624 begin
/*****************************************************************************
 *
 * Filename:
 * ---------
...
#define RG_Ratio_Typical 0x14a
#define BG_Ratio_Typical 0x130
...
static char *sensor_name = "HI553_ST";
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .mipi_settle_delay_mode = MIPI_SETTLEDELAY_MANUAL,//0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL //hisense  changed 
...
{
	LOG_INF("lihaiyan  sensor_id = %d \n", (read_cmos_sensor(0x0f16) << 8) | read_cmos_sensor(0x0f17));
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	LOG_INF("lihaiyan framerate = %d, min framelength should enable? \n", framerate,min_framelength_en);
...
write_cmos_sensor(0x0046, 0x0100);	
	...
	LOG_INF("Exit! lihaiyan shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
void HI553_Sensor_calc_wbdata(void)
{
    ...
    hi553_otp_info.r_gain = (0x100 * RG_Ratio_Typical) / rg_value;
    hi553_otp_info.b_gain = (0x100 * BG_Ratio_Typical) / bg_value;
...
static void sensor_init(void)
{
	...
	// Validation/Project
	//====================================================================
	LOG_INF("lihaiyan start hi553 sensor_init");
	//====================================================================
	// software code 
	...
	LOG_INF("lihaiyan end hi553 sensor_init");

}	/*	sensor_init  */
static void preview_setting(void)
{
	LOG_INF("E lihaiyan preview_setting\n");
...
{
	LOG_INF("E! lihaiyan capture_setting currefps:%d\n",currefps);
	if (currefps == 300) {
//Sensor Information////////////////////////////
//Sensor	  : Hi-553
//Date		  : 2015-09-05
...
{
	LOG_INF("E! normal_video_setting currefps:%d\n",currefps);
	preview_setting(); // zhaozhensen@wind-mobi.com 20170324 for lowlight
}
...
//dingyisheng@wind-mobi.com 20160715 begin
    if (enable) {
      write_cmos_sensor(0x0A04,0x0101);
       write_cmos_sensor(0x0200,0x0102);
    } else {
...
{
	LOG_INF("Terry-------Enter %s\n",__func__);

LOG_INF("hi553 lihaiyan [get_imgsensor_id] ");
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
            ...
            *sensor_id = return_sensor_id();
				LOG_INF("after read ---Sensor_id	= %x\n", *sensor_id);		
				if (*sensor_id == imgsensor_info.sensor_id) {				
                	LOG_INF("lihaiyan hi553 i2c write id: 0x%x, sensor id: 0x%x \n  ", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}	
            	LOG_INF("lihaiyan hi553 Read sensor id fail, write id: 0x%x, id: 0x%x     retry = %d\n", imgsensor.i2c_write_id,*sensor_id,retry);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
	}
	LOG_INF("lihaiyan hi553 have not found");
	if (*sensor_id != imgsensor_info.sensor_id) {
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF 
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("lihaiyan hi553 *sensor_id = 0xFFFFFFFF");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
		LOG_INF("Terry-------Exit %s  \n,__func__");
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            LOG_INF("lihaiyan hi553 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
{
	LOG_INF("lihaiyan hi553 close E");
...
{
	LOG_INF("lihaiyan hi553 preview E");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("lihaiyan hi553 capture E");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_INF("lihaiyan hi553 get_info scenario_id = %d\n", scenario_id);
...
static kal_uint32 control(MSDK_SCENARIO_ID_ENUM scenario_id, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("lihaiyan [contrlo]scenario_id = %d", scenario_id);
	spin_lock(&imgsensor_drv_lock);
...
UINT32 HI553_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	LOG_INF("lihaiyan hi553 HI553_MIPI_RAW_SensorInit");
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	HI841_MIPI_RAW_SensorInit	*/
// zhaozhensen@wind-mobi.com 20160624 end
