...
#include "s5k3l8mipiraw_Sensor_new.h"
...
#define PFX "S5K3L8"
#define LOG_1 pr_debug("S5K3L8,MIPI 1LANE\n")
...
/****************************   Modify end    *******************************************/
 // zhaozhensen@wind-mobi.com 20160619 begin
#ifdef CONFIG_MTK_CAM_CAL
extern void read_s5k3l8_eeprom_mtk_fmt(void);
extern u8 read_s5k3l8_eeprom_lens_id(void);
extern u8 read_s5k3l8_eeprom_module_id(void);
#endif
 // zhaozhensen@wind-mobi.com 20160619 end
static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_SENSOR_ID_NEW,  //Sensor ID Value: 0x30C9//record sensor id defined in Kd_imgsensor.h
  ...
  .custom5 = {
	...
	//yutao@wind-mobi.com modefy the i2c addr from 5a to 20 begin
	.i2c_addr_table = {0x20, /*0x20,0x5a*/0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
   //yutao@wind-mobi.com modefy the i2c addr from 5a to 20 end
...
{
//add by lifengxiang@wind-mobi.com 20601111 begin
    kal_uint32 sensor_id = 0;
    
    sensor_id = (read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001);
    if(sensor_id == 0x30C8)
    {
        if(read_s5k3l8_eeprom_lens_id() == 0x48)
        {
            pr_err("lifengxiang_s5k3l8 read lens id successful.\n");
            sensor_id += 1;
        }
    }
    
    return sensor_id;
//add by lifengxiang@wind-mobi.com 20601111 end
}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			// zhaozhensen@wind-mobi.com 20160619 begin
			#ifdef CONFIG_MTK_CAM_CAL
				read_s5k3l8_eeprom_mtk_fmt();
			#endif
			// zhaozhensen@wind-mobi.com 20160619 end
				pr_debug("i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
                return ERROR_NONE;
            }
			pr_debug("Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	int i=0;
	for(i=0; i<10; i++){
		pr_debug("delay time = %d, the frame no = %d\n", i*10, read_cmos_sensor(0x0005));
		mdelay(10);
	}
	#endif
	return ERROR_NONE;
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		pr_debug("Warning:=== current_fps %d fps is not support, so use cap1's setting\n",imgsensor.current_fps/10);
		imgsensor.pclk = imgsensor_info.cap1.pclk;
		imgsensor.line_length = imgsensor_info.cap1.linelength;
		imgsensor.frame_length = imgsensor_info.cap1.framelength;  
		imgsensor.min_frame_length = imgsensor_info.cap1.framelength;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			S5K3L8_read_eeprom_new((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;	
        /******************** PDAF END   <<< *********/
        default:
            break;
...
UINT32 S5K3L8_MIPI_RAW_SensorInit_new(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
