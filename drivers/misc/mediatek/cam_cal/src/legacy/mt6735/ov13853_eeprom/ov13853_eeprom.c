...
static int ov13853_eeprom_read = 0;
static int eeprom_flag = 0;
//int read_ov13853_eeprom_size(u8 slv_id, u16 offset, u8* data,int size);
//kal_uint16 ov13853_read_cmos_sensor_eeprom(kal_uint32 addr);
typedef struct {
#if 0
...
OTP_DATA ov13853_eeprom_data = {{0}};
//u8 af_inf_data=0;
...
int read_ov13853_eeprom(u8 slv_id, u16 offset, u8* data)
{
    int ret = 0;
...
int read_ov13853_eeprom_size(u8 slv_id, u16 offset, u8* data,int size)
{
	int i = 0;
	for(i = 0; i < size; i++){
		if(read_ov13853_eeprom(slv_id, offset+i, data+i) != 0)
			return -1;
	}
	return 0;
}
void ov13853_write_cmos_sensor_eeprom(kal_uint32 addr, kal_uint32 para)
{
...
{
		int RG_Ratio_Typical = 0x103, BG_Ratio_Typical = 0x11B;
		int rg, bg, R_gain, G_gain, B_gain, Base_gain;
		// apply OTP WB Calibration
		CAM_CALDB("liukun ov13853 eeprom_flag =%d \n",eeprom_flag);
		if (eeprom_flag & 0x40) {
			rg = (ov13853_eeprom_data.Data[0x22])<<2 | (ov13853_eeprom_data.Data[0x21]);
			bg = (ov13853_eeprom_data.Data[0x24])<<2 | (ov13853_eeprom_data.Data[0x23]);
			//calculate G gain
			CAM_CALDB("liukun ov13853 RG_Ratio =%x,BG_Ratio=%x\n",rg,bg);
			...
			if (R_gain>0x400) {
				ov13853_write_cmos_sensor_eeprom(0x5056, R_gain>>8);
				ov13853_write_cmos_sensor_eeprom(0x5057, R_gain & 0x00ff);
			}
			if (G_gain>0x400) {
				ov13853_write_cmos_sensor_eeprom(0x5058, G_gain>>8);
				ov13853_write_cmos_sensor_eeprom(0x5059, G_gain & 0x00ff);
			}
			if (B_gain>0x400) {
				ov13853_write_cmos_sensor_eeprom(0x505a, B_gain>>8);
				ov13853_write_cmos_sensor_eeprom(0x505b, B_gain & 0x00ff);
			}
		}
	}

u8 read_ov13853_eeprom_vendor_id(void)
{
  CAM_CALDB("liukun ov13853 vendor id =%x \n",ov13853_eeprom_data.Data[4]);
  return ov13853_eeprom_data.Data[4];
}
...
static long ov13853eeprom_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] ov13853_eeprom_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
{
    memcpy((void *)data,(void *)&ov13853_eeprom_data.Data[offset],size);
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, ov13853_EEPROM_DEVICE_ID,ptempbuf->u4Length);
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = ov13853eeprom_Ioctl_Compat,
#endif
...
static int __init CAM_CAL_init(void)
{
  ...
  //  i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALDB("failed to register ov13853_eeprom driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALDB("failed to register ov13853_eeprom driver, 2nd time\n");
        return -ENODEV;
    }
