...
//auto FW_UPDATE select among  BOE, HLT and other modules.
	...
	{
		#include "E280L_HLT_Wind_E411_C0F_20161109.i"//HLT FW update!
	};
	static unsigned char i_CTPM_FW_HLT_ILI9881[]=
	{
		#include "E280L_HLT_Wind_E411_ILI9881_C33_20161215.i"//HLT FW update!
	};
//zhangkaiyuan@wind-mobi.com 20170118 begin	
	static unsigned char i_CTPM_FW_HLT_FL11281[]=
	{
		#include "E280L_HLT_Wind_E411_FL11281_C50_20170213.i"//HLT FW update!
	};
//zhangkaiyuan@wind-mobi.com 20170118 end
#endif 
...
//sunsiyuan@wind-mobi.com at 20160125 begin
#ifdef CONFIG_WIND_DEVICE_INFO
extern u16 g_ctp_fwvr;
extern u16 g_ctp_vendor;
extern char g_ctp_id_str[21];
#endif
//sunsiyuan@wind-mobi.com at 20160125 end
...
static int i_update_FW(void)
{
	...
	if(private_ts->vendor_sensor_id == 0x12) {
	    ...
	    printk("wind-log   himax852xes_probe lcd_flag = %d\n",lcd_flag);
		if (lcd_flag == 1) { 
			ImageBuffer = i_CTPM_FW_HLT_HX8394F; //helitai_module
			fullFileLength = sizeof(i_CTPM_FW_HLT_HX8394F); 
			printk("wind-log   himax852xes_probe #5\n");
		} else if (lcd_flag == 2){
			ImageBuffer = i_CTPM_FW_HLT_ILI9881; //helitai_module
			fullFileLength = sizeof(i_CTPM_FW_HLT_ILI9881); 
			printk("wind-log   himax852xes_probe #6\n");
		} else if (lcd_flag == 3){
			ImageBuffer = i_CTPM_FW_HLT_FL11281; //helitai_module
			fullFileLength = sizeof(i_CTPM_FW_HLT_FL11281); 
...
int fts_ctpm_fw_upgrade_with_i_file_flash_cfg(struct himax_config *cfg)
{
	...
	if(private_ts->vendor_sensor_id == 0x12) {
		...
		if (lcd_flag == 1) {
			ImageBuffer = i_CTPM_FW_HLT_HX8394F; //helitai_module
			printk("wind-log   himax852xes_probe #7\n");
		} else if (lcd_flag == 2) {
			ImageBuffer = i_CTPM_FW_HLT_ILI9881;
			printk("wind-log   himax852xes_probe #8\n");
		} else if (lcd_flag == 3) {
			ImageBuffer = i_CTPM_FW_HLT_FL11281;
		}else {
			ImageBuffer = NULL;  //unknown_module
		}
	}  else {
	...
	do
	{
		...
		if (private_ts->debug_log_level & BIT(2)) {
			...
			if (atomic_read(&private_ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(private_ts->SMWP_enable)) {
				...
				//dingyisheng@wind-mobi.com 20161107 begin
				//wake_lock_timeout(&private_ts->ts_SMWP_wake_lock, TS_WAKE_LOCK_TIMEOUT);
				wake_lock(&private_ts->ts_SMWP_wake_lock);
				//dingyisheng@wind-mobi.com 20161107 end
				msleep(200);
		...
		//dingyisheng@wind-mobi.com 20161107 begin
			wake_unlock(&private_ts->ts_SMWP_wake_lock);
		//dingyisheng@wind-mobi.com 20161107 end
				continue;
			}
#endif
...
//	char *lcd_name = NULL; //added by hebiao at 2016905
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
  printk("wind-log   himax852xes_probe #1\n");
//hebiao@wind-mobi.com 2016906 begin
	//lcd_name = find_lcm_name();

  
  //_parse_tag_videolfb();
  printk("wind-log   himax852xes_probe %s #2\n", tp_lcm_name);
  
	if (strcmp(tp_lcm_name, "hx8394f_hd720_dsi_vdo_boe") == 0) {
		lcd_flag = 1;
		 printk("wind-log   himax852xes_probe #2\n");
	} 
//zhangkaiyuan@wind-mobi.com 20170118 begin	
	else if (strcmp(tp_lcm_name, "fl11281_hd720_dsi_vdo_hlt") == 0) {
		lcd_flag = 3;
	}
//zhangkaiyuan@wind-mobi.com 20170118 end	
	else {
		lcd_flag = 2;
		 printk("wind-log   himax852xes_probe #3\n");
	}
	
//hebiao@wind-mobi.com 2016906 end	
printk("wind-log   himax852xes_probe #3\n");
...
#endif
#ifdef HX_SMART_WAKEUP
//ranyanhao@wind-mobi.com 20160323
/*
...
//dingyisheng@wind-mobi.com 20170317 begin
	async_cookie_t fake_cookie = 0;

	I("Himax 852xES touch panel driver init\n");

	/* use syncrhonous call to avoid circumstances that
	 * tpd_probe() happens before the invocation of    
	 *  himax852xes_init_sync(), which causes the problem
	 * driver lost.
	 */

//	async_schedule(himax852xes_init_async, NULL);
	himax852xes_init_async(NULL, fake_cookie);
