...
static int tpd_irq_registration(void)
{
	...
	//printk("hb tpd_irq_registration\n");
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	//int i = 0;  //added by hebiao 2016913
				...
				switch (detected_gestures[2]) {		
					...
					case 0x35:
						...
						if (rmi4_data->gesture_cust_en[6] == 1) {
							printk("gesture z\n");
							KEY_EVENT = KEY_CUST_06; //z
						}
						break;
					case 0x7:
						//KEY_EVENT = KEY_CUST_09; //s
						if (rmi4_data->gesture_cust_en[9] == 1) {
							printk("hebiao s\n");
							KEY_EVENT = KEY_CUST_09;
						}
						break;
					case 0x26:
						//KEY_EVENT = KEY_CUST_10; //v
						if (rmi4_data->gesture_cust_en[10]== 1) {
							printk("gesture v\n");
							KEY_EVENT = KEY_CUST_10;  //v
						}
						break;
					case 0x77:
						...
						if (rmi4_data->gesture_cust_en[11] == 1) {
							//printk("gesture gesture w\n");
							KEY_EVENT = KEY_CUST_11; //w
						}
						break;
					case 0x65:
						//KEY_EVENT = KEY_CUST_12; 
						if (rmi4_data->gesture_cust_en[12] == 1) {
							//printk("gesture e\n");
							KEY_EVENT = KEY_CUST_12; //e
						}
						break;
					default:
						break;
//hebiao@wind-mobi.com 20160913 end
				}
			}
			if (KEY_EVENT) {
				//printk("hb KEY_EVENT=0x%x\n", KEY_EVENT);
...
#endif
{
	...
	tpd->tpd_dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6735-touch");
	if (tpd->tpd_dev->of_node)
	{
		printk("of_find_compatible_node-->mediatek,mt6735-touch\n");

		//tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");
		tpd->reg = regulator_get(tpd->tpd_dev, "vgp1"); //modified by hebiao 2016907
		//ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);
		ret = regulator_set_voltage(tpd->reg, 3300000, 3300000);	//modified by hebiao 2016907
		if (ret) {
