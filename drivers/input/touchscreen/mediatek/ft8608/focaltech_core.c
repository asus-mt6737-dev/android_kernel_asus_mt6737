...
//zhangkaiyuan@wind-mobi.com 20161121 begin
#if FTS_GESTRUE_EN
#define FOCALTCH_PROC_TOUCH_FOLDER "android_touch"
#define FOCALTCH_PROC_SMWP_FILE "SMWP"
#define FOCALTCH_PROC_GESTURE_FILE "GESTURE"
static struct proc_dir_entry *focaltch_proc_GESTURE_file = NULL;
static struct proc_dir_entry *focaltch_touch_proc_dir = NULL;
static struct proc_dir_entry *focaltch_proc_SMWP_file = NULL;
static struct fts_ts_data  *private_ts = NULL; 
static int GESTURE_SEND_FLAG = 0;
#endif
//zhangkaiyuan@wind-mobi.com 20161121 end
...
//zhangkaiyuan@wind-mobi.com 20161121 begin
#if FTS_GESTRUE_EN
static ssize_t focaltch_SMWP_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
	struct fts_ts_data  *ts = private_ts; 
	size_t ret = 0;
...
static ssize_t focaltch_SMWP_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	struct fts_ts_data  *ts = private_ts; 
    char buf[80] = {0};
...
static struct file_operations focaltch_proc_SMWP_ops =
{
	.owner = THIS_MODULE,
	.read = focaltch_SMWP_read,
	.write = focaltch_SMWP_write,
};
static ssize_t focaltch_GESTURE_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
	struct fts_ts_data *ts = private_ts;
	size_t ret = 0;
...
static ssize_t focaltch_GESTURE_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	struct fts_ts_data  *ts = private_ts; 
	int i = 0;
...
static struct file_operations focaltch_proc_Gesture_ops =
{
	.owner = THIS_MODULE,
	.read = focaltch_GESTURE_read,
	.write = focaltch_GESTURE_write,
};
static int focaltch_touch_proc_init(void)
{
	focaltch_touch_proc_dir = proc_mkdir(FOCALTCH_PROC_TOUCH_FOLDER, NULL);
	if (focaltch_touch_proc_dir == NULL)
	{
		printk(" %s: focaltch_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}
	focaltch_proc_SMWP_file = proc_create(FOCALTCH_PROC_SMWP_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		focaltch_touch_proc_dir, &focaltch_proc_SMWP_ops);
	if(focaltch_proc_SMWP_file == NULL)
	{
		printk(" %s: proc SMWP file create failed!\n", __func__);
		goto fail_0;
	}
	focaltch_proc_GESTURE_file = proc_create(FOCALTCH_PROC_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		focaltch_touch_proc_dir, &focaltch_proc_Gesture_ops);
	if(focaltch_proc_GESTURE_file == NULL)
	{
		printk(" %s: proc GESTURE file create failed!\n", __func__);
		goto fail_1;
	}
	return 0 ;
	fail_0: remove_proc_entry( FOCALTCH_PROC_SMWP_FILE, focaltch_touch_proc_dir );
	fail_1: remove_proc_entry( FOCALTCH_PROC_GESTURE_FILE, focaltch_touch_proc_dir );	
	return -ENOMEM;

}
static void focaltch_touch_proc_deinit(void)
{
		    remove_proc_entry( FOCALTCH_PROC_GESTURE_FILE, focaltch_touch_proc_dir );
		    remove_proc_entry( FOCALTCH_PROC_SMWP_FILE, focaltch_touch_proc_dir );
			remove_proc_entry( FOCALTCH_PROC_TOUCH_FOLDER, NULL );
}
#endif
//zhangkaiyuan@wind-mobi.com 20161121 end
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	char data;
	int err = 0;
	struct fts_ts_data *fts_data = NULL;//zhangkaiyuan@wind-mobi.com 20161121
...
//sunjingtao@wind-mobi.com 20160617 end
	...
	if (!fts_data) {
		printk("%s: Failed to alloc mem for fts_data\n",__func__);
		return -ENOMEM;
	}
	private_ts = fts_data;
...
reset_proc:
	...
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
		...
		kfree(fts_data);//zhangkaiyuan@wind-mobi.com 20161123
		return -1;
	}
//sunjingtao@wind-mobi.com 20160617 begin
	tpd_irq_registration();
//sunjingtao@wind-mobi.com 20160617 end
	...
	tpd_load_status = 1;
//zhangkaiyuan@wind-mobi.com 20161123 begin
#if FTS_GESTRUE_EN
    focaltch_touch_proc_init();
#endif
//zhangkaiyuan@wind-mobi.com 20161123 end
...
*/   
	...
	#ifdef TPD_AUTO_UPGRADE
		printk("********************wind_tp Enter CTP Auto Upgrade********************\n");
		is_update = true;
...
/*#ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
...

	{
	   ...
	   //Get TP type
		...
		#endif
		//huyunge@wind-mobi.com 20160907 start
		if((ver==0xA6)||(ver==0xA7))//boyi yashi
		{
		    	 vendor_name="FT3427";
			 #ifdef CONFIG_WIND_DEVICE_INFO
			 sprintf(g_ctp_id_str,"FT3427");
			 #endif	 
		}
		else if(0x8D==ver)//tianma
		{
		     	vendor_name="FT3327";
			 #ifdef CONFIG_WIND_DEVICE_INFO
			  sprintf(g_ctp_id_str,"FT3327");
			 #endif
		}else if(FTS_Vendor_3_ID==ver)//FT3427
		{
		     	vendor_name="FT3427";
			 #ifdef CONFIG_WIND_DEVICE_INFO
			  sprintf(g_ctp_id_str,"FT3427");
		...
		//huyunge@wind-mobi.com 20160907 end
...
static void tpd_resume(struct device *h)
{
	...
	#endif
    private_ts->suspended = false; //zhangkaiyuan@wind-mobi.com 20161121
...
static void tpd_suspend(struct device *h)
{
	...
	#endif
    private_ts->suspended = true; //zhangkaiyuan@wind-mobi.com 20161121
#ifdef CONFIG_MTK_SENSOR_HUB_SUPPORT
	int sem_ret;
