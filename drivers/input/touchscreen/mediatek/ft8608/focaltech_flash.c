...
static unsigned char CTPM_FW[] = {
	#include "FT3427_YD_E281_GX_VID0xe4_V0d_D02_20170301_app.i"
};
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	int	fw_filenth2 = sizeof(CTPM_FW2);//huyunge@wind-mobi.com 20160819
	if((CTPM_FW[fw_filenth-12] == 30) || (CTPM_FW1[fw_filenth1-12] == 30) || (CTPM_FW2[fw_filenth2-12] == 30)) //tuwenzan@wind-mobi.com modify at 20160504   huyunge@wind-mobi.com 20160819
	{
		is_5336_fwsize_30 = 1;
	}
	else 
...
int fts_ctpm_get_i_file_ver(u8 tp_vendor_id)
{
	...
	ui_sz2 = sizeof(CTPM_FW2); //huyunge@wind-mobi.com 20160819

		if(tp_vendor_id==0xA6)
		{
			fw_id = CTPM_FW[ui_sz - 2];
		}
		else if (tp_vendor_id==FTS_Vendor_3_ID)  //0XF0 //lihaiyan@wind-mobi.com 20160825
		{
			fw_id = CTPM_FW2[ui_sz2 - 2];//huyunge@wind-mobi.com 20160819
		}
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	printk("********************wind_tp Enter CTP Auto Upgrade555555555555555555555555********************\n");
		...
		{
		    printk("********************wind_tp Enter CTP Auto Upgrade666666666666666666666666********************\n");
			printk("twz vendor_name=FT3427 \n");
			...
			pbt_buf = CTPM_FW2;
			/*call the upgrade function*/
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW2));
			if (i_ret != 0)
			{
					dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
			}
		...
		//huyunge@wind-mobi.com 20160819 end
		else if(tp_vendor_id==0x8D)  //sunjingtao@wind-mobi.com 20160825
		{
		//	printk("twz vendor_name=FT3327 \n");
			...
			pbt_buf = CTPM_FW1;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW1));
			if (i_ret != 0)
			{
					dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
			}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("********************wind_tp Enter CTP Auto Upgrade 111111111111********************\n");
	printk("[FTS] uc_tp_fm_ver = 0x%x, uc_tp_vendor_id = 0x%x\n",uc_tp_fm_ver, uc_tp_vendor_id);

	if((uc_tp_vendor_id != FTS_Vendor_3_ID))//huyunge@wind-mobi.com 20160811 add FTS_Vendor_3_ID 
	{
		uc_boot_vendor_id = ft5x46_ctpm_VidFWid_get_from_boot(client);
		printk("[FTS] uc_boot_vendor_id= 0x%x!\n", uc_boot_vendor_id);
		if((uc_boot_vendor_id == FTS_Vendor_1_ID)|| (uc_boot_vendor_id == FTS_Vendor_2_ID)|| (uc_boot_vendor_id == FTS_Vendor_3_ID))//huyunge@wind-mobi.com 20160811 add FTS_Vendor_3_ID
		{
	...
	{
	    printk("********************wind_tp Enter CTP Auto Upgrade222222222222222222222********************\n");
		msleep(100);
